...
import { ServerErrorBuilder } from "./ServerErrorBuilder";
...
>({
  ...
  > = async (options) => {
    ...
    try {
      ...
      if (!res.ok) {
        const serverError = ServerErrorBuilder.buildServerResponseError(
          res.status.toString(),
          res.body ? await res.text() : undefined
        );
        if (serverError.code && errorHandlers[serverError.code]) {
          return errorHandlers[serverError.code](serverError);
        } else {
          throw serverError;
        }
      }
      ...
      if (ServerErrorResponse.isErrorResponseSchema(data)) {
        throw ServerErrorBuilder.buildServerResponseError(
          res.status.toString(),
          data.message
        );
      }
      ...
      if (ServerError.isServerError(e)) {
        throw e;
      }
      console.error(e);
      throw ServerErrorBuilder.buildServerResponseError("999");
    }
  };
